...
class DeployController extends Controller
{
    ...
    public function show($id)
    {
        switch ($id) {
            ...
            case 'rdp':
                $vms = $this->vmRepository->getByTipo('rdp');
                $clientes = $this->clienteRepository->getClientesComApelido();
                return view('deploy.rdp')->with('vms', $vms)->with('clientes', $clientes);
            case 'EscalaWeb':
    ...
    public function rdp(Request $request) {
        $cliente = $request->input('cliente');
        $vm = $request->input('vm');
        $nome_servico = $request->input('nome');
        ...
        $taskId = $this->asyncTasksRepository->create('DeployRDP', $dado);

        Rdp::dispatch($clienteDados, $dados, $escalaServer, $nome_servico, $taskId);
